...
class RecordListViewModel extends AutoDisposeNotifier<RecordList?> {
  ...
  Future<void> init() async {
    try {
      ...
      Map<String, dynamic> resBody = await recordRepository.findAll(type: 1);
      if (!resBody['success']) {
  ...
  Future<void> nextList() async {
    ...
    Map<String, dynamic> responseBody =
        await recordRepository.findAll(page: state!.pageNumber + 1, type: 1);
...
final recordListProvider =
    NotifierProvider.autoDispose<RecordListViewModel, RecordList?>(
  () => RecordListViewModel(),
);
